const API_URL = "/auth/api/students"; // Ruta de Flask para la API de estudiantes
...
// Función para obtener estudiantes desde la API de Flask
async function fetchStudents() {
    try {
        const response = await fetch('/auth/api/students');
        if (!response.ok) {
...
// Función para crear estudiante en la API de Flask
...
async function updateStudent(studentId, studentData) {
    try {
        const response = await fetch(`/auth/api/students/${studentId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
// Función para agregar entrada al historial de QR
async function addQRHistory(studentId, actionType, qrContent) {
    try {
        const response = await fetch('/auth/api/qr-history', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Error al agregar historial');
        }
        
        return await response.json();
    } catch (error) {
        console.error('Error adding QR history:', error);
